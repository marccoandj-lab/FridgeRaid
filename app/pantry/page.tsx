    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 pb-16 pt-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="font-heading text-3xl font-bold text-foreground sm:text-4xl">Pantry</h1>
            <p className="mt-1 text-sm text-muted-foreground">{items.length} item{items.length !== 1 ? 's' : ''} stored</p>
          </div>
          <button
            onClick={() => setShowForm(!showForm)}
            className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-black transition-all hover:bg-amber-400"
          >
            <Plus size={16} />
            Add item
          </button>
              ...
              <div className="space-y-3 rounded-2xl border border-amber-500/20 bg-card p-4">
                ...
                <div className="flex gap-3">
                  ...
                  <select
                    ...
                    className="rounded-xl border border-foreground/10 bg-background px-3 py-2.5 text-sm text-foreground focus:border-amber-500/40 focus:outline-none"
                  >
                    {categories.map((c) => (
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1 rounded-xl border border-foreground/10 bg-background px-4 py-2.5 text-sm text-foreground focus:border-amber-500/40 focus:outline-none"
                  />
                  <button onClick={handleAdd} className="rounded-xl bg-amber-500 px-6 py-2.5 text-sm font-semibold text-black transition-all hover:bg-amber-400">
                    Save
                  </button>
                </div>
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => removeItem(item.id)} className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-red-500/20 hover:text-red-400">
                            <Trash2 size={12} />
                          </button>
                        </div>
                      </motion.div>
