    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 pb-16 pt-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="font-heading text-3xl font-bold text-foreground sm:text-4xl">Shopping List</h1>
            <p className="mt-1 text-sm text-muted-foreground">{uncheckedItems.length} item{uncheckedItems.length !== 1 ? 's' : ''} to buy</p>
          </div>
          <div className="flex gap-2">
            {checkedItems.length > 0 && (
              <button onClick={clearChecked} className="inline-flex items-center gap-1.5 rounded-xl border border-foreground/10 px-3 py-2.5 text-xs text-muted-foreground transition-colors hover:text-foreground">
                <CheckCheck size={14} />
                Clear done
              </button>
            )}
            <button onClick={() => setShowForm(!showForm)} className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-black transition-all hover:bg-amber-400">
              <Plus size={16} />
              Add
            </button>
          </div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            <div className="flex gap-3 rounded-2xl border border-amber-500/20 bg-card p-3">
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Item name"
                className="flex-1 rounded-xl border border-foreground/10 bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-amber-500/40 focus:outline-none"
              ...
              <input
                ...
                placeholder="Qty"
                className="w-24 rounded-xl border border-foreground/10 bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-amber-500/40 focus:outline-none"
              />
              <button onClick={handleAdd} className="rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-black hover:bg-amber-400">Add</button>
            </div>
          </motion.div>
        )}
          ...
          <div className="space-y-1">
            {uncheckedItems.map((item, i) => (
              <motion.div key={item.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}>
                <button onClick={() => toggleItem(item.id)} className="flex w-full items-center gap-3 rounded-xl bg-card p-3 text-left ring-1 ring-foreground/5 transition-all hover:ring-amber-500/20">
                  ...
                  <span onClick={(e) => { e.stopPropagation(); removeItem(item.id) }} className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-red-500/20 hover:text-red-400">
                    <Trash2 size={12} />
                  </span>
                </button>
              </motion.div>
