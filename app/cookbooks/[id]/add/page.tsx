...
export default function AddRecipePage() {
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 pb-24 pt-4 sm:pb-24 sm:pt-8">
      ...
      <div className="mb-6 flex gap-1.5 rounded-xl bg-card p-1.5 ring-1 ring-foreground/5 sm:mb-8">
        {(['meals', 'custom'] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 rounded-lg px-4 py-2.5 text-sm font-medium transition-colors active:scale-[0.97] ${
              tab === t
