...
export function CreateCookbookForm({ onSubmit }: CreateCookbookFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await onSubmit(name.trim()); setName(""); } catch { setError("Failed to create"); } finally { setIsSubmitting(false); }
  };

  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-amber-500/10 bg-card p-4">
      <h3 className="mb-3 flex items-center gap-2 text-sm font-bold text-foreground"><BookOpen size={16} className="text-amber-500" />New Cookbook</h3>
      <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Cookbook name..."
      ...
      <button type="submit" disabled={isSubmitting}
        ...
        <Plus size={15} />{isSubmitting ? "Creating..." : "Create"}</button>
    </form>
  );
}
