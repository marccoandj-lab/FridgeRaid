...
export function JoinCookbookForm({ onSubmit }: JoinCookbookFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ok) { setCode(""); } else { setError("Invalid invite code"); }
    } catch { setError("Failed to join"); } finally { setIsSubmitting(false); }
  };

  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-amber-500/10 bg-card p-4">
      <h3 className="mb-3 flex items-center gap-2 text-sm font-bold text-foreground"><Hash size={16} className="text-amber-500" />Join Cookbook</h3>
      <input type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="Enter invite code..." maxLength={8}
      ...
      <button type="submit" disabled={isSubmitting}
        ...
        <UserPlus size={15} />{isSubmitting ? "Joining..." : "Join"}</button>
    </form>
  );
}
